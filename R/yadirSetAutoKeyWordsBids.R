yadirSetAutoKeyWordsBids <- function(KeywordIds             = NULL,
                                     AdGroupIds             = NULL,
                                     CampaignIds            = NULL,
  ...
  if ( all(is.null(TargetTrafficVolume), 
           is.null(SearchIncreasePercent),
           is.null(SearchBidCeiling),
           is.null(TargetCoverage),
           is.null(NetworkIncreasePercent),
           is.null(NetworkBidCeiling)) ) {
    stop("Set any bidding rule!")
  }
  ...
  # create body shell
  query_list <- list(method = "setAuto",
                     params = 
  ...
  for (part_of_id in id_parts) {
    ...
    if ( object == "keywords" ) {
      ...
      query_list_param <- lapply( part_of_id, function(x) list(KeywordId   = x,
        ...
        query_list_param <- lapply( part_of_id, function(x) list(KeywordId   = x,
                                                                 BiddingRule = 
                                                                   list(NetworkByCoverage = 
                                                                          list(TargetCoverage  = TargetCoverage,
                                                                               IncreasePercent = NetworkIncreasePercent,
                                                                               BidCeiling      = ifelse( is.null(NetworkBidCeiling), NA, NetworkBidCeiling * 1000000 ))))
        )
      }
