yadirStartCampaigns <-  function(Login = NULL, 
  ...
  if(is.null(Ids)){
    stop("В аргумент Ids необходимо передать вектор содержаший Id кампаний по которым необходимо возобновить показ объявлений. Вы не передали ниодного Id.")
  }
  ...
  if(TotalCampStoped %in% c(2,3,4) & !(TotalCampStoped %% 100 %in% c(12,13,14))){
    out_message <- "кампании запущены"
  } else if(TotalCampStoped %% 10 == 1 & TotalCampStoped %% 100 != 11){
    out_message <- "кампания запущена"
  } else {
    out_message <- "кампаний запущено"
  }
